...
class DS2Autoscaler(Autoscaler):
    ...
    def ds2_scaling(self):
        ...
        for wall_time, num_rows in zip(delta_wall_time_list, delta_num_processed_rows_list):
            if wall_time > 0:
                unit_throughput_list.append(num_rows / wall_time)
            else:
                # If wall_time is 0, the operator hasn't processed anything in this window
                # Use a very small throughput to avoid division by zero
                logger.warning(
                    f"Delta wall time is 0 for an operator (delta_rows={num_rows}). "
                    f"Using fallback throughput of 0.001 rows/s."
                )
                unit_throughput_list.append(0.001)
